...
Web 浏览器接入：**`motifd` 自带 Web UI 静态资源**（Vite + React 构建产物经
`rust-embed` 嵌入二进制），在同一监听器上以 `GET /` + `GET /assets/*` 暴露；
...
- **Web 前端**：Vite + Solid + xterm.js + diff2html + Shiki，构建产物经 `rust-embed` 嵌入到 `motifd` 二进制（`crates/motif-server/src/embed.rs`），同端口暴露 `GET /` 与 `GET /assets/*`。
...
- **Workspace 布局**：单 cargo workspace，核心 crate：
  ...
  motif/
    ...
    web/                # Vite + React 前端项目，构建产物被 motif-server 嵌入
  ```
