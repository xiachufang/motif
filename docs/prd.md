...
motif-cast  --motifd-url wss://host:7777/  [-- 命令]
```

Web 浏览器接入：**`motifd` 自带 Web UI 静态资源**（Vite + Solid 构建产物经
`rust-embed` 嵌入二进制），在同一监听器上以 `GET /` + `GET /assets/*` 暴露；
浏览器直接打开 `https://host:7777/` 即可，没有单独的 `motif-web` 桥接进程。
...
- **Workspace 布局**：单 cargo workspace，核心 crate：
  ...
  motif/
    ...
    web/                # Vite + Solid 前端项目，构建产物被 motif-server 嵌入
  ```

  非 proto crate 自己负责 `src/lib.rs`（库代码）+ `src/main.rs`（薄入口）。`motifd`
  同时讲 motif HTTP/WS JSON-RPC 协议**和**给浏览器吐 Web UI 静态资源，没有独立的
  Web 桥接进程。
