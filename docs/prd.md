...
┌─────────────────┐                            │  │  - PTY pool (N shells) │  │
│  Web (browser)  │ ─────────────────────────► │  │  - file ops            │  │
└─────────────────┘     (Web UI 静态资源       │  │  - git ops             │  │
                        从 motifd 同端口下发)  │  │  - subscriber list     │  │
                                               │  └────────────────────────┘  │
                                               │  ┌────────────────────────┐  │
                                               │  │  Session 2, 3, ...     │  │
                                               │  └────────────────────────┘  │
...
# 客户端 —— TUI（默认进入交互式 session 列表）
motif-tui                                          # 自动探测本机 ws://127.0.0.1:7777
motif-tui   --host host:7777                       # 远端 motifd
motif-tui   --via ssh://user@host                  # 透 SSH 隧道
motif-tui   list-servers --prefix motifd-          # tailnet 上发现 motifd 节点

# 终端共享（一次性把本机终端 cast 到 motifd 上）
motif-cast                                         # 自动探测 ws://127.0.0.1:7777
motif-cast  --host host:7777  [-- 命令]
```

启动 `motif-tui` 后进入 ratatui picker：列表浏览 server 上的 session，
`Enter` 进入；`n` 行内新建（输入 name + workdir）；`d` 删除带确认；`r`
刷新；`q` 退出。`Enter` 后 picker 把当前 `ConnectedV2` 交给 attach UI，
detach (`Ctrl-g d`) 即退出进程，与 tmux 语义一致。所有 session 管理动作
都在 picker 里完成 —— CLI 不再有 `list / new / destroy / attach` 子命令。

Web 浏览器接入：**`motifd` 自带 Web UI 静态资源**（Flutter Web 构建产物经
`rust-embed` 嵌入二进制），在同一监听器上以 `GET /` + Flutter Web 运行时资源暴露；
浏览器直接打开 `https://host:7777/` 即可。

拆分理由：

- **依赖不混杂**：`motifd` 不携带 TUI 依赖；`motif-tui` 不携带 server-side PTY/git 实现。
- **部署模型清晰**：核心服务端常以 daemon/systemd 跑 `motifd`，开发者只装 `motif-tui`，浏览器用户什么客户端都不用装。
- **Native GUI 未来直连 motifd**：协议平级，与 `motif-tui` 同款连接，不经任何桥接。
...
- 文本/小文件：内联 base64 走 `fs.read`，10MB 上限。
- 二进制大文件：v1 不专门优化；超出上限直接拒绝，client 自行 fallback（例如让用户在 PTY 里 `scp`）。
...
JSON-RPC 2.0 拆分到三条 HTTP/WS 端点：控制面是 HTTP POST `/rpc/<method>`（短请求），
事件推送和 PTY 字节流各自是独立 WS（`/events`、`/pty/<pty_id>`）。所有端点共用
Bearer token 鉴权，并通过 `X-Motif-Session` header（浏览器场景退化为 `?session=`
查询参数）做有状态绑定。**完整 wire 协议参考见 [`rpc.md`](./rpc.md)**：

| 章节 | 内容 |
| --- | --- |
| §1 Transport | `/rpc/<method>` + `/events` + `/pty/<id>` 端点、Bearer 鉴权、帧格式、`seq` 回放 |
...
| §7 推送事件 | tree / git / client / pty / view 全套（shell-integration block 由 client 解析） |
...
`motifd` 在 `--listen` 之外可选 `--tailscale`，让自身以 tsnet 节点身份直接落到
tailnet 上，省去外置 Tailscale daemon。motif client 默认提供两条"开箱即用"的接入路径，外加直连：

| 路径 | 适合 | 详细文档 |
|---|---|---|
| **直连**（`wss://host:port/`） | 局域网、公网 VPS（配 TLS 证书）、SSH 已开好的隧道 | 本文档 §3、§4.8 |
| **Tailscale 嵌入** | 个人多设备、跨区域、NAT 穿透；client 二进制即 tailnet 节点（server 也可选嵌入），无需装 daemon | [`tailscale.md`](./tailscale.md) |
| **SSH 隧道** | 只有 SSH 可用的内网、跳板机 / bastion；client 自动 spawn `ssh -L`，复用 ssh_config / agent | [`ssh-tunnel.md`](./ssh-tunnel.md) |

设计原则：

- **隧道能力都集中在 `motif-net` crate**，server 和 client 共用同一套实现。Tailscale 用 cargo feature 控制（关掉后退回纯 Rust 二进制 + 直连），不强制依赖 Go 工具链以外的运行时。
- **motifd 不内置反代 / 多租户路由逻辑**。需要这类复杂部署的用户继续用现成工具（Cloudflare Tunnel、nginx 反代）即可。
- **接入路径的选择由 client 端决定**。server 只感知"在哪些 listener 上接受连接"。
...
- **Web 前端**：Flutter Web + libghostty-vt WASM，构建产物经 `rust-embed` 嵌入到 `motifd` 二进制（`crates/motif-server/src/embed.rs`），同端口暴露 `GET /`、Flutter 运行时资源与 motif 协议端点。
- **Git**：fork `git` CLI，不引 `git2`（避免 libgit2 链接复杂度）。
- **Workspace 布局**：单 cargo workspace，核心 crate：

  ```
  motif/
    crates/
      motif-proto/      # 协议类型（server / client 共享）
      motif-net/        # 监听器 / 拨号器；TCP + 可选 Tailscale backend
      motif-tailscale/  # libtailscale FFI 封装（stub 或 bundled feature）
      motif-server/     # 核心服务端；产物 motifd（同时挂 Web UI 静态资源）
      motif-client/     # 客户端共享库（transport、coordinator、shell parser）
      motif-tui/        # TUI 客户端；产物 motif-tui
      motif-cast/       # 把本机终端 cast 到 motifd 上的一次性会话；产物 motif-cast
    apps/flutter/       # Flutter 客户端；build/web 构建产物被 motif-server 嵌入
  ```

  非 proto crate 自己负责 `src/lib.rs`（库代码）+ `src/main.rs`（薄入口）。`motifd`
  同时讲 motif HTTP/WS JSON-RPC 协议**和**给浏览器吐 Web UI 静态资源。

  二进制依赖图基本不重叠：`motifd` 不拉 `ratatui`/`crossterm`/`vte`；`motif-tui`/`motif-cast`
  不拉 `axum`/`portable-pty`/server-side git 逻辑（这部分只在 `motif-server` 里）。
...
| **M7: Web client** | Web UI 静态资源嵌入 `motifd` + `web/` 前端 |
| **M8: Shell integration** | 私有 OSC 777 + bootstrap 脚本 + BlockStore |

每个里程碑结束做一次端到端 demo。详细规划见对应专题文档：[`tailscale.md`](./tailscale.md)、[`ssh-tunnel.md`](./ssh-tunnel.md)、[`web-client.md`](./web-client.md)、[`shell-integration.md`](./shell-integration.md)。
