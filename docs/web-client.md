...
- ❌ **PWA / 离线 / 后台 push 通知**（Service Worker + Web Push）。响应式做到能用即可。
  *已有*的是 live 通道：Claude 钩子的 `notification` 事件会弹应用内 toast（标签可见时）
  或浏览器桌面通知（标签隐藏/失焦时，需授权 + https/localhost 安全上下文）；
  每会话静音开关在设置面板（纯客户端，见 `docs/rpc.md` §7.6）。
