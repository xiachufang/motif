...
`GetBlockOutputParams` / `GetBlockOutputResult`）继续放在
`crates/motif-proto/src/pty.rs:1`，由客户端代码构造，不再有对应的 server
RPC。
...
- **可关闭**：`MOTIF_SHELL_INTEGRATION=0` 跳过注入
  ...
  管道；客户端 5s 超时后把 shell 标成 `Unknown`，不再等 block 事件
...
历史脚注：服务端原先也跑同一份状态机，并通过 `/events` 把
`pty.output` / `pty.prompt_started` 等事件广播给所有客户端；Phase 5b
（见 `crates/motif-proto/src/event.rs:1`）拆解后服务端不再解析 OSC，状态机
迁到客户端，事件由客户端 coordinator 本地合成
（`crates/motif-client/src/coordinator.rs:392`），仍以 JSON-RPC notification
形式喂给上层 TUI / Web，wire 形态保持兼容。
...
- **空 Enter / Ctrl-C 取消编辑 / fish autosuggestion repaint 都走"redraw"
  ...
  结果是这些事件在 UI 上完全静默,不再像旧版那样为每次按键产出一张空 block
  卡片。下游真有命令运行时会走 `Composing → 777;C` 的正常路径生成 block。
...
## 5. BlockStore（历史 / 已迁出服务端）

旧版服务端为每个 PTY 维护一份 ring buffer，把走过 `777;D` 或被强制
finalize 的 block 存起来，并通过 `pty.list_blocks` / `pty.get_block_output`
两个 RPC 让晚到的客户端回填。Phase 5b 拆解后服务端不再解析 OSC，这两个
RPC 也一起删了（见 `crates/motif-server/src/rpc.rs:103` 的脚注）；服务端
只保留每 PTY 2 MB 的原始字节 ring（`crates/motif-server/src/pty.rs:44`），
晚到客户端用 `WS /pty/<id>?since=N` 拉过去的字节流，按 §4 的状态机自己重放
得到 block 历史。

block 在客户端的存活策略由各客户端自定（TUI 当前是按需在内存里维持，未
持久化），共享 wire 形态仍由 `BlockSummary` / `GetBlockOutputResult` 等
proto 类型描述
（`crates/motif-proto/src/pty.rs:67`），供未来本地 RPC / IPC 复用。
...
- 结构化壳（未来 Web / 任何 xterm 派生）按 `(block_id, scope)` 把字节路由到
  ...
  为 HTML —— 与实时观看完全一致。服务端不再提供单独的 block 回填 RPC。
