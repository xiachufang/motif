...
客户端若在 `/pty` 上启用 framed zlib，会先按 `rpc.md` 解 frame / zlib，再把还原后
的同一份 PTY bytes 喂给终端和 OSC 状态机。
各客户端吃同一份字节流，按本地状态机拼出 *block*（命令 + 输出 +
...
（in-process Rust 枚举，不是 JSON-RPC 通知）；调用方（如 motif-tui 的
`apply_shell_event` 或 Flutter client）直接消费这些值驱动本地 UI。PTY 字节走
`/pty/<id>` WebSocket 二进制帧，每客户端自行决定怎么渲染——
没有"服务端先解析再合成事件"的中间层。
...
回填 RPC。晚到 / 重连的客户端用 `WS /pty/<id>?since=N` 拉 raw delta 或 VT
snapshot；若启用了 framed zlib，先解码成 PTY bytes，再按 §4 的状态机自己重放成
block。
