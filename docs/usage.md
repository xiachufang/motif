...
motifd 通过 WSS 把 owner JWT 发给 relay，relay 按 JWT `sub` 聚合限速。启动打印
一个 rzv 形态的 `motif://pair` 链接和 QR。App 扫码/粘贴即可通过 relay 连接
（外层 WSS + 端到端 TLS pin + psk bearer）；relay 只转发内层加密字节。同时再开一个
...
3. 选择连接类型：
   ...
   - SSH：可自动在远端安装/启动 motifd，再通过 SSH local forward 连接。
   - WSL（Windows App）：复用 SSH bootstrap 在 WSL 中安装/启动 Linux motifd，再通过
     localhost 连接。
...
- [`windows-wsl.md`](./windows-wsl.md)：Windows 原生 server 与 WSL bootstrap 连接。
