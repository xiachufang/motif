...
- `prompt` / `command` / `output` 对应 FinalTerm 的 `133;A..133;B` /
  `133;B..133;C` / `133;C..133;D` 三个区段；这三种 scope 时 `block_id`
  非空，按 (block_id, scope) 路由到对应 block 渲染器。
- `passthrough` 表示字节不属于任何 block —— spawn 后到首次 `133;A` 之间
  的 fish welcome banner / SSH MOTD,`133;D` 到下一次 `133;A` 之间的
