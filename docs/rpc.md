...
| `/pty/<id>` | binary 帧 = 原始 stdin 字节，写到 PTY 主端。 | binary 帧 = master 输出原始字节；不包任何 envelope。Replay 也是同一条裸字节流，服务端可拆成多个 binary 帧发送。close code 见 §1.6。 |
...
- **PTY 回放**：`/pty/<id>?since=<bytes>` 走的是字节游标，不是事件 seq。每
  个 PTY 维护 2 MB 字节环（`pty.rs::RING_BYTES`）。`since` 可省略。带 `since`
  时的状态码：
  - `since == total` → 不回放直接进 live。
  - `origin <= since < total` → 把 ring 内 `since..total` 推下去，然后进 live。
    当前实现按 64 KiB 左右拆成多个 binary 帧，wire 上仍然只是连续原始字节。
  - `since < origin` → close `4011`（history truncated）。
  - `since > total` → close `4012`（stale cursor）。
  - **省略 `since`**（tail 请求）→ 服务端在锁内取当前 `origin`，把 ring 内
    `origin..total` 推下去再进 live。永远不会 4011/4012。
- **Meta 帧**：每个（非关闭）`/pty/<id>` 连接的**第一帧**是一个 WebSocket
  **Text** 帧 `{"since":<offset>}`，`offset` 是紧随其后的第一个数据字节的绝对
  字节偏移（带 `since` 时即被采纳的 `since`，tail 时是解析出的 `origin`）。其余
  数据帧一律是 Binary，客户端按帧类型区分。客户端把 `offset` 采纳为自己的
  cursor —— 由于它是服务端在连接当刻算出的，到客户端记录时也不会过期（无重连竞态）——
  之后按收到的 binary frame byte count 累加即可增量续传。
- **4011/4012 恢复**：客户端清空本地 scrollback，**不带 `since`** 重连（即退化为
  tail 请求），再通过 meta 帧重新得到一个精确 cursor。
- **PTY replay/live 同步点**：服务端在同一把 PTY state lock 下完成 ring 检查/
  origin 读取、replay 复制和 live receiver 创建；reader 线程也在同一把锁下 append
  ring 并 broadcast live bytes。因此 replay 与 live 边界不漏字节、不重复字节，
  且 tail 的 `origin` 与 replay 快照原子一致。
...
推荐客户端把 `session` 状态、结构化事件和 PTY raw bytes 分开处理：
...
8. **输入与 resize**：PTY 的 stdin 走 `/pty` binary frame，这是唯一写入路径。
   写入只针对 active PTY（其 raw WS 已打开）；在流连接前的极短窗口内到达的输入
   会被丢弃，而不是另起一条 HTTP 调用。终端尺寸变化继续走 `pty.resize`。
9. **4011 / 4012**：收到 `/pty` close `4011` 或 `4012` 说明本地 cursor 已无法
   和服务端 ring 对齐。客户端应丢弃该 PTY 的 terminal surface / shell parser
...
类型定义在 `crates/motif-proto/src/event.rs`。共 **12 条** 事件,客户端按 method
