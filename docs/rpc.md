...
历史上的统一 `/ws` 端点和 `/blob/<transfer_id>` 通道在重构里都已删除；blob
传输能力整体下线。
...
| `-32016` | `BlockNotFound` | 保留码位；当前无 RPC 触发（block 解析已下放到客户端） |
| `-32099` | `Internal` | 内部分类不下来的错误 |

`error.data` 是可选 `serde_json::Value`，目前未约定结构，仅做诊断用途。
原 blob 系列错误码（`-32011..-32015`）随 blob 通道一起下线，已从 enum 中
移除。
...
> 服务端不暴露 block 枚举 / 回放 RPC（无 `pty.list_blocks` /
> `pty.get_block_output`）。block 的所有划分、缓存、回放都由客户端基于
> `/pty/<id>` 的原始字节流完成；详见 §6 与 `shell-integration.md`。
...
> 历史上的 `fs.openBlob` / `fs.commitBlob` / `fs.cancelBlob` 和配套 `/blob/<transfer_id>`
> 通道已整体下线，没有替代方法 — 大文件读写当前仅依赖 `fs.read` / `fs.write`。
...
类型定义在 `crates/motif-proto/src/event.rs`。当前共 **11 条** 事件 + 一个
`Unknown` 前向兼容 fallback。Phase 5b 起所有 shell-integration 派生事件
（`pty.output` / `pty.cwd_changed` / `pty.shell_bootstrapped` /
`pty.prompt_started` / `pty.prompt_ended` / `pty.command_started` /
`pty.command_finished` / `pty.shell_context`）整体下线 —— 客户端从
`/pty/<id>` 字节流自己解析 OSC（见 §6 与 `shell-integration.md`）。
