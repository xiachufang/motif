...
下游构建是分层透传的：`motif-net` → `motif-client` → `motif-tui`，以及 `motif-net` → `motif-server` → `motif-embed`，各自暴露 `tailscale` 与 `tailscale-bundled` 两个 feature。motifd（`motif-server`）**默认开启** `tailscale-bundled`（`default = ["tailscale-bundled"]`），所以开箱即支持 `--tailscale`，构建机需要 Go。用 `--no-default-features` 构建则**完全不链接 tailscale**（连 motif-tailscale 都不编、无需 Go，`--tailscale*` 参数也随之消失）——硬化审核镜像（`deploy/review/Dockerfile`）正是这么做的。
