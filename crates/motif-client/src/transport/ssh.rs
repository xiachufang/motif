...
impl SshTunnel {
    pub async fn open(target: &str, remote_port: u16) -> anyhow::Result<Self> {
        which::which("ssh").map_err(|_| anyhow!(
            "system `ssh` binary not found in PATH; install OpenSSH or use --via direct"
        ))?;
        ...
        cmd.arg("-N")
            ...
            .arg("-L").arg(format!("{local_port}:127.0.0.1:{remote_port}"));
        ...
        let mut child = cmd.spawn().with_context(|| format!("spawning ssh -L {local_port}:127.0.0.1:{remote_port} {user_host}"))?;
        ...
        loop {
            if let Ok(Some(status)) = child.try_wait() {
                ...
                anyhow::bail!("ssh exited before tunnel ready (status={status}): {}", buf.trim());
            }
