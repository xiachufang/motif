...
//! [`ShellEvent`]s. The [`crate::coordinator::Coordinator`]
//! pipes each `/pty/<id>` byte burst through a
//! [`motif_proto::terminal_query::QueryScanner`] and this state
//! machine; the events surface as JSON notifications alongside server-
//! pushed `/events` frames.
//!
//! Ported verbatim from the (now-deleted) `motif-server/src/shell/state.rs`
//! in Phase 5b of the protocol redesign. The server stopped parsing
//! shell-integration OSC and now passes the raw bytes through; each
//! client builds its own block model from the same wire.
//!
...
#[derive(Debug)]
pub enum BlockState {
    /// PTY just spawned; haven't seen any shell-integration marker yet.
    Unknown,

    /// Got prompt-start. Shell is rendering its prompt; bytes after this
    ...
    /// Got prompt-end. User is typing; bytes after this boundary are
    ...
    /// Got command-start. Command is executing; bytes are appended to `output`.
    Running {
        block_id: BlockId,
        cmd: String,
        cwd: PathBuf,
...
#[derive(Debug)]
pub enum ShellEvent {
    /// First successful shell-integration marker, or 5s timeout (kind = Unknown).
    Bootstrapped,
    /// Emitted on every prompt-start marker. `block_id` is the id allocated for
    /// this prompt cycle — same as the previous `PromptStarted` for a
    /// pure redraw, new for a fresh cycle.
    PromptStarted {
        block_id: BlockId,
    },
    /// Emitted on the AtPrompt → Composing transition.
    PromptEnded {
        block_id: BlockId,
    },
    CommandStarted {
    ...
    /// A shell-integration cwd property said cwd changed. Reader loop dedupes against
...
pub struct ShellState {
    ...
    /// Tracked separately because cwd properties fire outside any block too.
    pub current_cwd: Option<PathBuf>,
    /// Explicit command text normally arrives just before command-start.
    /// Stash text and consume it on the next command-start marker so a
    /// missing explicit command marker doesn't block the transition.
    ...
    pub fn on_osc(&mut self, q: &QueryKind) -> Vec<ShellEvent> {
        ...
        match q {
            ...
            QueryKind::Osc133CmdStart { cmdline_url } => {
                ...
                match prev {
                    BlockState::Composing {
                        ...
                        // fish-shell `reader/reader.rs:858-862`). Prefer
                        // it when present; fall back to the explicit
                        // command marker from Motif's private protocol.
                        let cmd = cmdline_url
                            .clone()
                        ...
                        // used here. (Without this, a late explicit
                        // command marker emitted during this cycle would
                        // survive into the next `Composing → Running`
                        // transition.)
                        self.pending_cmd = None;
            ...
            QueryKind::Osc133CmdEnd { exit } => {
                self.first_osc_seen(&mut out);
                // Belt-and-suspenders: any pending command text left here
                // (e.g. a late explicit command marker arrived after a
                // native 133;C had already consumed cmdline_url) must not
                // leak across the block boundary into the next cycle's
...
mod tests {
    ...
    fn happy_path_command_lifecycle_keeps_block_id() {
        ...
        assert!(
            s.active_block_id().is_none(),
            "no block id between 133;D and next 133;A"
        );
    ...
    fn composing_to_prompt_start_is_redraw_no_synthesis() {
        ...
        assert!(
            !evs.iter()
                .any(|e| matches!(e, ShellEvent::CommandFinished { .. })),
        ...
        assert_eq!(
            id, id_first,
            "real command keeps the same id across the redraw"
        );
    ...
    fn cwd_change_dedupes() {
        let mut s = new_state();
        s.on_osc(&QueryKind::Osc133PromptStart);
        let evs = s.on_osc(&QueryKind::Osc7Cwd {
            path: "/tmp".into(),
        });
        assert!(!evs
            .iter()
    ...
    fn cmdline_url_takes_priority_over_pending_cmd() {
        ...
        // `fish_preexec`, Motif's explicit command marker fires INSIDE the event
        ...
        // Motif's explicit command marker + private C arrive AFTER, while we're already
    ...
    fn pending_cmd_falls_back_when_cmdline_url_absent() {
        // bash/zsh + our bootstrap: there's no native cmdline_url; we emit
        // explicit command text then command-start from inside preexec. The
        // state machine must still pick up the cmd from pending_cmd.
        let mut s = new_state();
        s.on_osc(&QueryKind::Osc133PromptStart);
        s.on_osc(&QueryKind::Osc133PromptEnd);
        s.on_osc(&QueryKind::Osc7770Cmd {
    ...
    #[test]
    fn prompt_start_clears_late_pending_cmd_on_force_finalize() {
        let mut s = new_state();
        s.on_osc(&QueryKind::Osc133PromptStart);
        s.on_osc(&QueryKind::Osc133PromptEnd);
        s.on_osc(&QueryKind::Osc133CmdStart {
            cmdline_url: Some("sleep 30".into()),
        });

        // Fish/native ordering can leave Motif's explicit command marker
        // arriving after the native command-start. If the command then
        // force-finalizes via prompt-start (no D), that late pending value
        // must not feed the next block.
        s.on_osc(&QueryKind::Osc7770Cmd {
            text: "sleep 30".into(),
        });
        assert!(s.pending_cmd.is_some());
        s.on_osc(&QueryKind::Osc133PromptStart);
        assert!(s.pending_cmd.is_none());
