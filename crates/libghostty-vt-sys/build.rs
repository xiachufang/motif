...
fn build_vendored(link_mode: LinkMode) {
    ...
    // Locate ghostty source: an explicit override remains useful for package
    // builds, while Motif defaults to the checkout shared with Flutter.
    let ghostty_dir = match env::var("GHOSTTY_SOURCE_DIR") {
        ...
        Err(_) => {
            let p = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../apps/flutter/ghostty");
            assert!(
                p.join("build.zig").exists(),
                "shared Ghostty checkout is missing at {}; initialize the apps/flutter/ghostty submodule",
                p.display()
            );
            p
        }
