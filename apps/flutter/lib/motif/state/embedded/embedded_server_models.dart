/// Immutable embedded-server configuration and protocol DTOs.
///
/// These values can cross persistence/FFI boundaries but contain no reactive
/// machinery. Runtime observability lives in `embedded_server_state.dart`.
library;
...
class EmbeddedServerStatus {
  ...
  ({String host, int port})? get loopbackEndpoint {
    for (final address in boundAddrs) {
      final hostPort = address.startsWith('tcp://')
          ? address.substring(6)
          : null;
