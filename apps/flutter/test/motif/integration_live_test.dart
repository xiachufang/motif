...
void main() {
  ...
  test('ping + list + attach + receive pty output', () async {
    ...
    // shell parser.
    if (ptyId != null) {
      expect(received, contains('pty.output'),
  ...
  test('write → server → pty.output round-trip (own session)', () async {
    final rpc = RpcClient()..connect(host: host, port: port, token: '');
    try {
      await rpc.ping();
    } catch (e) {
    ...
    final sub = rpc.events.listen((e) {
      if (e.method == 'pty.output') {
        final b64 = e.params['data_b64'] as String?;
        if (b64 != null) output.write(String.fromCharCodes(base64Decode(b64)));
      }
    });
